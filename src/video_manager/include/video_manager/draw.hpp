...
drawBoundingBox(const cv::Mat &frame,
                const std::vector<common_msgs::msg::Detection> &detections,
                int current_target_id, cv::Scalar color = cv::Scalar(0, 0, 255),
  ...
  for (const auto &detection : detections) {
    ...
    if (current_target_id == detection.id) {
      curr_color = cv::Scalar(0, 255, 0); // B, G, R
    } else {
      curr_color = color;
    }
    // Draw the bounding box rectangle
    cv::rectangle(image, cv::Point(detection.x, detection.y),
                  cv::Point(detection.x + detection.width,
