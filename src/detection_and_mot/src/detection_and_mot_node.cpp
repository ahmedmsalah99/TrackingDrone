...
using my_kd_tree_t = nanoflann::KDTreeSingleIndexAdaptor<
        nanoflann::L2_Simple_Adaptor<float, DetPointCloud>,
        DetPointCloud, 2 /* dim */
        >;
...
public:
    DetectionAndMOTNode() : Node("detection_and_mot_node"), current_target_(6) {
        ...
        detection_tree_ = std::make_unique<my_kd_tree_t>(2,curr_det_point_cloud_,nanoflann::KDTreeSingleIndexAdaptorParams(100) );
...
private:
    ...
    void frameCallback(const shm_msgs::msg::Image1m::ConstSharedPtr& msg) {
        ...
        if(current_detections_stamp_.nanoseconds() != 0 && new_dets_available_){
            ...
            if(time_now > current_detections_stamp_ && (time_now - current_detections_stamp_) < delay_time){
                ...
                Eigen::Matrix<float, Eigen::Dynamic, 6> data(current_detections_.size(),6);
                {
    ...
    std::shared_ptr<common_msgs::srv::ChangeTarget::Response> response) {
        ...
        if(ret_index.empty()){
            ...
            response->success = false;
            response->message = "Unexpected result when trying to change target. No near targets found.";
            return;
        }
    ...
    bool initCurrentTarget(){
        ...
        if(ret_index.size() == 0){
            RCLCPP_ERROR(get_logger(), "Tried to reset current target but no results were found.");
            return false;
        }
    ...
    void updateCurrentTarget(){
        ...
        if(ret_index.size() == 0){
            // failed to update (unexpected state)
            // 
            RCLCPP_ERROR(get_logger(), "Unexpected result when trying to update current target. No near targets found.");
            current_target_(4) = -1;
            return;
        }
    ...
    void searchClosestTargets(const Eigen::RowVectorXf& query_det,int& num_results, std::vector<uint32_t>& ret_index, std::vector<float>& out_dist_sqr){
        float query_pt[2] = {query_det(0) + (query_det(2) - query_det(0)) / 2.0f, query_det(1) + (query_det(3) - query_det(1)) / 2.0f};
        num_results = detection_tree_->knnSearch(
